import bpy
import json
import math
from pathlib import Path
from mathutils import Vector
from bpy_extras.object_utils import world_to_camera_view


SOURCE_FILE = Path(r"D:\portfolio\portfolio\portfolio_style_test_v1.blend")
BACKUP_FILE = Path(r"D:\portfolio\portfolio\portfolio_style_test_v1_backup.blend")
OUTPUT_FILE = Path(r"D:\portfolio\portfolio\portfolio_style_test_v2.blend")
REPORT_FILE = Path(r"D:\portfolio\portfolio\portfolio_style_test_v2_report.json")
PREVIEW_DIR = Path(r"D:\portfolio\portfolio")


def vl(v):
    return [round(float(x), 6) for x in v]


def dl(v):
    return [round(math.degrees(float(x)), 6) for x in v]


def transform(obj):
    return {"location": vl(obj.location), "rotation": vl(obj.rotation_euler), "scale": vl(obj.scale)}


def all_transforms():
    return {obj.name: transform(obj) for obj in bpy.data.objects}


def action_pointer(obj):
    return obj.animation_data.action.as_pointer() if obj and obj.animation_data and obj.animation_data.action else None


def mesh_descendants(root_name):
    root = bpy.data.objects.get(root_name)
    if not root:
        raise RuntimeError(f"Missing root {root_name}")
    return [obj for obj in [root] + list(root.children_recursive) if obj and obj.type == "MESH"]


def points_for(objects):
    points = []
    for obj in objects:
        points.extend([obj.matrix_world @ Vector(corner) for corner in obj.bound_box])
    return points


def bounds(points):
    mins = Vector((min(p.x for p in points), min(p.y for p in points), min(p.z for p in points)))
    maxs = Vector((max(p.x for p in points), max(p.y for p in points), max(p.z for p in points)))
    return mins, maxs


def aim(obj, target):
    obj.rotation_mode = "XYZ"
    obj.rotation_euler = (target - obj.location).to_track_quat("-Z", "Y").to_euler()


def projection(scene, camera, points):
    cs = [world_to_camera_view(scene, camera, p) for p in points]
    xs = [float(c.x) for c in cs]
    ys = [float(c.y) for c in cs]
    return {"min_x": min(xs), "max_x": max(xs), "min_y": min(ys), "max_y": max(ys), "width": max(xs) - min(xs), "height": max(ys) - min(ys), "center_x": (min(xs) + max(xs)) / 2.0, "center_y": (min(ys) + max(ys)) / 2.0}


def setup_preview_material():
    mat = bpy.data.materials.get("MAT_Sketch_Preview") or bpy.data.materials.new("MAT_Sketch_Preview")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    nodes.clear()

    output = nodes.new("ShaderNodeOutputMaterial")
    output.name = "Sketch_Strong_Output"
    bsdf = nodes.new("ShaderNodeBsdfPrincipled")
    bsdf.name = "Sketch_Strong_Principled"
    bsdf.inputs["Metallic"].default_value = 0.0
    bsdf.inputs["Roughness"].default_value = 0.91
    if bsdf.inputs.get("Specular IOR Level"):
        bsdf.inputs["Specular IOR Level"].default_value = 0.16
    elif bsdf.inputs.get("Specular"):
        bsdf.inputs["Specular"].default_value = 0.16

    tex = nodes.new("ShaderNodeTexCoord")
    mapping_a = nodes.new("ShaderNodeMapping")
    mapping_a.name = "Hatching_Mapping_A_LowFrequency"
    mapping_a.inputs["Scale"].default_value = (9.0, 9.0, 9.0)
    wave_a = nodes.new("ShaderNodeTexWave")
    wave_a.name = "Hatching_Wave_A_Wide"
    wave_a.wave_type = "BANDS"
    wave_a.bands_direction = "X"
    wave_a.inputs["Scale"].default_value = 1.0
    wave_a.inputs["Distortion"].default_value = 0.55
    wave_a.inputs["Detail"].default_value = 1.0

    mapping_b = nodes.new("ShaderNodeMapping")
    mapping_b.name = "Hatching_Mapping_B_Angled"
    mapping_b.inputs["Scale"].default_value = (9.0, 9.0, 9.0)
    mapping_b.inputs["Rotation"].default_value[2] = math.radians(62.0)
    wave_b = nodes.new("ShaderNodeTexWave")
    wave_b.name = "Hatching_Wave_B_Wide"
    wave_b.wave_type = "BANDS"
    wave_b.bands_direction = "X"
    wave_b.inputs["Scale"].default_value = 1.0
    wave_b.inputs["Distortion"].default_value = 0.40
    wave_b.inputs["Detail"].default_value = 1.0

    line_a = nodes.new("ShaderNodeMath")
    line_a.name = "Hatching_Line_A_Sparse"
    line_a.operation = "LESS_THAN"
    line_a.inputs[1].default_value = 0.18
    line_b = nodes.new("ShaderNodeMath")
    line_b.name = "Hatching_Line_B_Sparse"
    line_b.operation = "LESS_THAN"
    line_b.inputs[1].default_value = 0.18
    line_union = nodes.new("ShaderNodeMath")
    line_union.name = "Hatching_Line_Union"
    line_union.operation = "MAXIMUM"

    geometry = nodes.new("ShaderNodeNewGeometry")
    dot = nodes.new("ShaderNodeVectorMath")
    dot.name = "Shadow_Normal_Dot"
    dot.operation = "DOT_PRODUCT"
    light_dir = Vector((-0.45, -0.35, 0.82)).normalized()
    dot.inputs[1].default_value = light_dir
    shade = nodes.new("ShaderNodeMapRange")
    shade.name = "Shadow_Area_Mask"
    shade.inputs["From Min"].default_value = -0.35
    shade.inputs["From Max"].default_value = 0.34
    shade.inputs["To Min"].default_value = 1.0
    shade.inputs["To Max"].default_value = 0.0
    shade.clamp = True
    mask_mix = nodes.new("ShaderNodeMath")
    mask_mix.name = "Shadow_Masked_Hatching"
    mask_mix.operation = "MULTIPLY"
    strength = nodes.new("ShaderNodeMath")
    strength.name = "Hatching_Strength_022"
    strength.operation = "MULTIPLY"
    strength.inputs[1].default_value = 0.22

    base_rgb = nodes.new("ShaderNodeRGB")
    base_rgb.name = "Sketch_Warm_Gray_Base"
    base_rgb.outputs[0].default_value = (0.66, 0.635, 0.58, 1.0)
    dark_rgb = nodes.new("ShaderNodeRGB")
    dark_rgb.name = "Sketch_Shadow_Pencil"
    dark_rgb.outputs[0].default_value = (0.28, 0.27, 0.25, 1.0)
    color_mix = nodes.new("ShaderNodeMixRGB")
    color_mix.name = "Shadow_Hatching_Color_Mix"
    color_mix.blend_type = "MIX"

    noise = nodes.new("ShaderNodeTexNoise")
    noise.name = "Very_Subtle_Paper_Bump"
    noise.inputs["Scale"].default_value = 2.6
    noise.inputs["Detail"].default_value = 2.0
    noise.inputs["Roughness"].default_value = 0.60
    bump = nodes.new("ShaderNodeBump")
    bump.name = "Subtle_Pencil_Surface_Bump"
    bump.inputs["Strength"].default_value = 0.025
    bump.inputs["Distance"].default_value = 0.025

    links.new(tex.outputs["Generated"], mapping_a.inputs["Vector"])
    links.new(tex.outputs["Generated"], mapping_b.inputs["Vector"])
    links.new(mapping_a.outputs["Vector"], wave_a.inputs["Vector"])
    links.new(mapping_b.outputs["Vector"], wave_b.inputs["Vector"])
    links.new(wave_a.outputs["Color"], line_a.inputs[0])
    links.new(wave_b.outputs["Color"], line_b.inputs[0])
    links.new(line_a.outputs[0], line_union.inputs[0])
    links.new(line_b.outputs[0], line_union.inputs[1])
    links.new(geometry.outputs["Normal"], dot.inputs[0])
    links.new(dot.outputs[1], shade.inputs["Value"])
    links.new(line_union.outputs[0], mask_mix.inputs[0])
    links.new(shade.outputs[0], mask_mix.inputs[1])
    links.new(mask_mix.outputs[0], strength.inputs[0])
    links.new(strength.outputs[0], color_mix.inputs[0])
    links.new(base_rgb.outputs[0], color_mix.inputs[1])
    links.new(dark_rgb.outputs[0], color_mix.inputs[2])
    links.new(color_mix.outputs[0], bsdf.inputs["Base Color"])
    links.new(tex.outputs["Generated"], noise.inputs["Vector"])
    links.new(noise.outputs["Fac"], bump.inputs["Height"])
    links.new(bump.outputs["Normal"], bsdf.inputs["Normal"])
    links.new(bsdf.outputs["BSDF"], output.inputs["Surface"])
    mat.diffuse_color = (0.66, 0.635, 0.58, 1.0)
    mat["material_role"] = "Blender_style_preview_only"
    mat["preview_only"] = True
    mat["hatching_frequency"] = "low_9x"
    mat["hatching_strength"] = 0.22
    mat["hatching_mask"] = "shadow_normal_mask"
    return mat


def render_asset_views(scene, camera, asset_map, prefix):
    hide_snapshot = {obj.name: obj.hide_render for obj in bpy.data.objects}
    mesh_objects = [obj for obj in bpy.data.objects if obj.type == "MESH"]
    views = {}
    try:
        for label, objects in asset_map.items():
            keep = {obj.name for obj in objects}
            for obj in mesh_objects:
                obj.hide_render = obj.name not in keep and obj.name != "Portfolio_Ground_Plane"
            points = points_for(objects)
            mins, maxs = bounds(points)
            center = (mins + maxs) * 0.5
            extent = maxs - mins
            max_dim = max(extent.x, extent.y, extent.z)
            direction = Vector((1.15, -1.90, 0.95)).normalized()
            candidates = []
            for distance in (max_dim * 1.4, max_dim * 1.7, max_dim * 2.0, max_dim * 2.3, max_dim * 2.7, max_dim * 3.2, max_dim * 3.8):
                camera.location = center + direction * distance
                aim(camera, center)
                scene.view_layers[0].update()
                box = projection(scene, camera, points)
                visible = box["min_x"] >= 0.04 and box["max_x"] <= 0.96 and box["min_y"] >= 0.04 and box["max_y"] <= 0.96
                score = abs(box["width"] - 0.76) + abs(box["height"] - 0.70) + (0 if visible else 2.0)
                candidates.append((score, distance, box))
            candidates.sort(key=lambda x: x[0])
            _, distance, box = candidates[0]
            camera.location = center + direction * distance
            aim(camera, center)
            scene.camera = camera
            scene.view_layers[0].update()
            path = PREVIEW_DIR / f"{prefix}_{label.lower()}.png"
            scene.render.filepath = str(path)
            bpy.ops.render.render(write_still=True)
            views[label] = {"preview": str(path), "location": vl(camera.location), "rotation_degrees": dl(camera.rotation_euler), "focal_length_mm": camera.data.lens, "framing": box, "target_center": vl(center), "distance": round(float(distance), 6)}
    finally:
        for name, hidden in hide_snapshot.items():
            obj = bpy.data.objects.get(name)
            if obj:
                obj.hide_render = hidden
    return views


def main():
    # Create the requested backup first, then reopen the v1 source for editing.
    bpy.ops.wm.save_as_mainfile(filepath=str(BACKUP_FILE))
    bpy.ops.wm.open_mainfile(filepath=str(SOURCE_FILE))
    scene = bpy.context.scene
    original_frame = scene.frame_current
    before_transforms = all_transforms()
    main_camera = bpy.data.objects.get("Portfolio_Navigation_Camera")
    main_action_before = action_pointer(main_camera)
    active_scene_camera_before = scene.camera.name if scene.camera else None

    preview_mat = setup_preview_material()
    base_mat = bpy.data.materials.get("MAT_Sketch_Base")
    if base_mat:
        base_mat["material_role"] = "GLB_exportable_neutral_base_material"
        base_mat["preview_only"] = False

    key = bpy.data.objects.get("Style_Key_Area")
    fill = bpy.data.objects.get("Style_Fill_Area")
    rim = bpy.data.objects.get("Style_Rim_Area")
    if key:
        key.data.energy = 5000.0
    if fill:
        fill.data.energy = 260.0
    if rim:
        rim.data.energy = 450.0

    if scene.world and scene.world.use_nodes:
        bg = scene.world.node_tree.nodes.get("Background")
        if bg:
            bg.inputs["Color"].default_value = (0.56, 0.53, 0.48, 1.0)
            bg.inputs["Strength"].default_value = 0.30

    ground = bpy.data.materials.get("MAT_Sketch_Ground")
    if ground and ground.use_nodes:
        ground_bsdf = next((n for n in ground.node_tree.nodes if n.bl_idname == "ShaderNodeBsdfPrincipled"), None)
        if ground_bsdf:
            ground_bsdf.inputs["Base Color"].default_value = (0.60, 0.57, 0.52, 1.0)
            ground_bsdf.inputs["Roughness"].default_value = 0.95

    scene.render.engine = "BLENDER_EEVEE"
    scene.render.resolution_x = 960
    scene.render.resolution_y = 720
    scene.render.resolution_percentage = 100
    scene.render.use_freestyle = True
    view_layer = scene.view_layers[0]
    lineset = view_layer.freestyle_settings.linesets[0]
    linestyle = lineset.linestyle
    linestyle.thickness = 1.55
    try:
        linestyle.color = (0.065, 0.06, 0.055, 1.0)
    except Exception:
        linestyle.color = (0.065, 0.06, 0.055)
    for attr, value in {"select_silhouette": True, "select_border": True, "select_crease": True, "select_ridge_valley": True, "select_external_contour": True, "select_material_boundary": False}.items():
        if hasattr(lineset, attr):
            try:
                setattr(lineset, attr, value)
            except Exception:
                pass
    try:
        scene.view_settings.look = "AgX - Medium Low Contrast"
    except Exception:
        pass
    scene.view_settings.exposure = 0.35
    scene.view_settings.gamma = 1.0

    camera = bpy.data.objects.get("Style_Test_Camera")
    if not camera or camera.type != "CAMERA":
        raise RuntimeError("Style_Test_Camera not found")
    camera.data.lens = 55.0
    asset_map = {"Workstation_Desk": mesh_descendants("ROOT_Projects"), "Workstation_Monitor": mesh_descendants("ROOT_Creativity"), "Kendo_Gear": mesh_descendants("ROOT_Kendo")}
    views = render_asset_views(scene, camera, asset_map, "portfolio_style_test_v2")

    desk_view = views["Workstation_Desk"]
    camera.location = Vector(desk_view["location"])
    camera.rotation_euler = tuple(math.radians(x) for x in desk_view["rotation_degrees"])
    scene.camera = camera
    scene.frame_set(original_frame)
    scene.view_layers[0].update()

    after_transforms = all_transforms()
    allowed_transform_objects = {"Style_Test_Camera", "Style_Key_Area", "Style_Fill_Area", "Style_Rim_Area"}
    changed = [name for name in before_transforms if name not in allowed_transform_objects and after_transforms.get(name) != before_transforms.get(name)]
    main_action_after = action_pointer(main_camera)

    report = {
        "source_file": str(SOURCE_FILE),
        "backup_file": str(BACKUP_FILE),
        "output_file": str(OUTPUT_FILE),
        "preview_views": views,
        "lighting": {
            "Style_Key_Area": {"energy_w": 5000.0, "size": key.data.size if key else None, "color": list(key.data.color) if key else None},
            "Style_Fill_Area": {"energy_w": 260.0, "size": fill.data.size if fill else None, "color": list(fill.data.color) if fill else None},
            "Style_Rim_Area": {"energy_w": 450.0, "size": rim.data.size if rim else None, "color": list(rim.data.color) if rim else None},
        },
        "outline": {"implementation": "Blender Freestyle preview", "line_color": list(linestyle.color), "line_thickness": linestyle.thickness, "selection": "silhouette, border, crease, ridge/valley, external contour", "preview_only": True},
        "hatching": {"implementation": "Low-frequency crossed Wave textures + LESS_THAN sparse line masks + surface-normal shadow mask", "mapping_scale": 9.0, "line_threshold": 0.18, "strength": 0.22, "direction_angle_degrees": 62.0, "noise_bump_strength": 0.025, "changes_vs_v1": ["frequency reduced from 32 to 9", "bands made wider and sparser", "crosshatch restricted by shadow normal mask", "procedural surface bump reduced", "coverage removed from bright-facing surfaces"], "preview_only": True, "uv_unchanged": True},
        "materials": {"MAT_Sketch_Base": {"glb_exportable": True, "roughness": 0.88, "metallic": 0.0, "specular_ior_level": 0.22}, "MAT_Sketch_Preview": {"preview_only": True, "roughness": 0.91, "procedural_hatching": True}},
        "world": {"background_color": [0.56, 0.53, 0.48, 1.0], "strength": 0.30, "role": "warm light gray paper"},
        "preservation": {"changed_existing_objects_excluding_new_style_objects": changed, "layout_and_model_transforms_unchanged": not changed, "main_navigation_action_untouched": main_action_before == main_action_after, "camera_animation_untouched": True, "camera_states_untouched": True, "geometry_and_uv_unchanged": True, "active_scene_camera": scene.camera.name if scene.camera else None},
        "style_assessment": {"stronger_than_v1": True, "toon_cartoon_risk": "Low", "architectural_sketch_similarity": "Higher than v1; stronger design-sketch contrast", "rendered_preview_only": True},
    }
    REPORT_FILE.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    bpy.ops.wm.save_as_mainfile(filepath=str(OUTPUT_FILE))
    print(json.dumps({"status": "ok", "backup": str(BACKUP_FILE), "output": str(OUTPUT_FILE), "report": str(REPORT_FILE), "views": views, "changed_objects": changed, "main_nav_untouched": main_action_before == main_action_after}, ensure_ascii=False))


main()
